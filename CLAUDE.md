...
- **Responsive** : 3 breakpoints (mobile < 480px, tablette < 768px, desktop). Sidebar desktop, topbar burger + drawer mobile (par defaut). Bottom nav disponible en alternative. Modals et drawers plein ecran sur mobile. Classes utilitaires : `wf-hide-mobile`, `wf-hide-tablet`, `wf-show-mobile`, `wf-show-desktop`, `wf-hide-desktop`, `wf-btn--block-mobile`.
...
Les composants qui apparaissent sur plusieurs ecrans (sidebar, burger menu) sont des **composants React** dans `src/components/` :

- `AppLayout.jsx` : Layout avec sidebar (desktop) + topbar burger + drawer (mobile). Wrapper pour ecrans authentifies.
- `CenteredLayout.jsx` : Layout centre pour ecrans d'auth (login, signup).
- `Sidebar.jsx` : Navigation sidebar desktop.
- `BottomNav.jsx` : Navigation mobile bottom bar (disponible, pas le defaut).
...
- Les composants partages (sidebar, burger menu) sont importes via React — toujours identiques partout
...
│   ├── styles/
│   │   └── wireframe.css      # Composants CSS (~170 composants, style wireframe)
│   ├── components/            # Composants React partages
│   │   ├── WfLink.jsx         # Navigation helper
│   │   ├── AppLayout.jsx      # Layout sidebar + burger menu mobile
│   │   ├── CenteredLayout.jsx # Layout centre (auth)
│   │   ├── Sidebar.jsx        # Sidebar desktop
│   │   └── BottomNav.jsx      # Bottom nav mobile (optionnel)
